...
class FactionInitializer:
    ...
    def __init__(self, width: int, height: int, world_state: WorldState) -> None:
        """
    ...
    def _generate_base_coords(self, player_count: int) -> list[Coord]:
        ...
        while len(coords) < player_count:
            ...
            if all(
                candidate.manhattan_distance(existing) >= required_distance
        ...
        if len(coords) < player_count:
            raise ValueError(
                "Unable to place all factions with required spacing. Try a larger map."
            )
    ...
    def _create_faction(
        self, index: int, base_coord: Coord, observer_mode: bool
    ) -> Faction:
        """
        ...
        if is_human:
            ...
            color_idx = (
                ((index + 1) % len(settings.colors.color_map)) + 1
                if observer_mode
                else (index % len(settings.colors.color_map)) + 1
            )
